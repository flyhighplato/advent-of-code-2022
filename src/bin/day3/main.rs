...
fn sum_rucksack_priorities1(lines: Vec<String>) -> u32 {
    return lines
        .iter()
        .map(|s| get_rucksack_priorities(s).iter().sum::<u32>())
        .sum();
}
...
fn sum_rucksack_priorities2(lines: Vec<String>) -> u32 {
    return lines
        .chunks(3)
        .map(|chunk| {
            let maybe_intersecting_priorities: Option<HashSet<u32>> =
                chunk
                    .iter()
                    .fold(None as Option<HashSet<u32>>, |accum, el| {
                        let item_set = HashSet::from_iter(el.chars().map(|c| char_to_priority(c)));

                        return match accum {
                            Some(ref set) => {
                                Some(HashSet::from_iter(set.intersection(&item_set).cloned()))
                            }
                            None => Some(item_set),
                        };
                    });
...
mod tests {
    ...
    fn both_different() {
        assert_eq!(
            get_rucksack_priorities(&"ab".to_string())
                .iter()
                .sum::<u32>(),
            0
        );
